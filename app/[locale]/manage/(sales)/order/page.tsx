...
export default function OrderListPage() {
    ...
    const handleCreateTag = async () => {
        ...
        try {
            const res = await axios.post("https://api.rock.io.vn/api/v1/orders", { name, description, slug });
            if (res) {
                mutate(`https://api.rock.io.vn/api/v1/orders?page=${currentPage}&limit=${limit}&search=${debouncedSearchTerm}`);
    ...
    const handleDeleteTag = async (tagId: string) => {
        try {
            const res = await axios.delete(`https://api.rock.io.vn/api/v1/orders/${tagId}`);
            if (res) {
                mutate(`https://api.rock.io.vn/api/v1/orders?page=${currentPage}&limit=${limit}&search=${debouncedSearchTerm}`);
    ...
    const [selectedUsers, setSelectedUsers] = useState<IOrder[]>([]);
    ...
    const { data, error, isLoading } = useSWR(
        `https://api.rock.io.vn/api/v1/orders?page=${currentPage}&limit=${limit}&search=${debouncedSearchTerm}`,
        fetcher, { revalidateIfStale: false, revalidateOnFocus: false, revalidateOnReconnect: false }
    );


    const roles: IOrder[] = data?.data;
    const totalPages = data?.pagination?.totalPages;
    const totalUsers = data?.pagination?.totalOrders;
    ...
    const copyToClipboardById = (tagId: string) => {
        copy(`#${tagId}`).then(() => {
            ...
            toast.success(`Đã sao chép Id đơn hàng: ${tagId}`);
        }).catch((error) => {
    ...
    const menuOptions = [
        { icon: <Copy size={16} strokeWidth={1.5} />, value: "copyId", label: "Copy ID đơn hàng", action: (tagId: string) => copyToClipboardById(tagId) },
        { icon: <ScanEye size={16} strokeWidth={1.5} />, value: "details", label: "Chi tiết đơn hàng", action: (tagId: string) => toast.error(`Tính năng đang được phát triển :))`) },
        { icon: <SquarePen size={16} strokeWidth={1.5} />, value: "edit", label: "Chỉnh sửa đơn hàng", action: (tagId: string) => toast.error(`Tính năng đang được phát triển :))`) },
        { icon: <Trash size={16} strokeWidth={1.5} />, value: "delete", label: "Xóa đơn hàng", action: (tagId: string) => handleDeleteTag(tagId) },
    ];
            ...
            <div className="text-black shadow rounded-md overflow-auto border select-none w-full bg-white">
                <div className="h-[55px] md:h-[60px] px-5 md:flex justify-between items-center w-full">
                    <div className="relative md:flex items-center hidden">
                        ...
                        <Input className="w-[360px] px-5 pl-10" type="text" placeholder="Tìm kiếm đơn hàng..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                    </div>
                ...
                <Table className="w-full">
                    <TableHeader>
                        <TableRow className="bg-gray-100 hover:bg-gray-20">
                            ...
                            <TableHead className="text-black px-4 h-[50px] font-bold text-[13px] whitespace-nowrap">Khách hàng</TableHead>
                            <TableHead className="text-black px-4 h-[50px] font-bold text-[13px] whitespace-nowrap">Khóa học</TableHead>
                            <TableHead className="text-black px-4 h-[50px] font-bold text-[13px] whitespace-nowrap">Thanh toán</TableHead>
                            <TableHead className="text-black px-4 h-[50px] font-bold text-[13px] whitespace-nowrap">Hình thức</TableHead>
                            <TableHead className="text-black px-4 h-[50px] font-bold text-[13px] whitespace-nowrap">Trạng thái</TableHead>
                            <TableHead className="text-black px-4 h-[50px] font-bold text-[13px] whitespace-nowrap">Ngày mua</TableHead>
                    ...
                    <TableBody>
                        {isLoading ? (
                            ...
                            roles.map((role: IOrder, index: number) => (
                                <TableRow key={role._id}>
                                    ...
                                    <TableCell className="h-[50px] px-4 cursor-pointer whitespace-nowrap">
                                        <h3 className="font-bold text-[13px] capitalize">Nguyễn Phương Nam</h3>
                                    </TableCell>
                                    <TableCell className="h-[50px] px-4 cursor-pointer whitespace-nowrap">
                                        2 khóa
                                    </TableCell>
                                    ...
                                    <TableCell className="h-[50px] px-4 cursor-pointer whitespace-nowrap">
                                        <div
                                            className={`rounded-lg px-2 py-1 text-xs w-min text-primary-foreground ${role.status === 'pending'
                                                    ? 'bg-yellow-400'
                                                    : role.status === 'paid'
                                                        ? 'bg-[#3eca65]'
                                                        : role.status === 'shipped'
                            ...
                            <TableRow> <TableCell colSpan={7} className="h-[50px] text-center font-medium"> Không có đơn hàng nào </TableCell></TableRow>
                        )}
                    </TableBody>
                </Table>
                <div className="h-[55px] px-5 flex justify-between items-center border-t w-full">
                    <div className="hidden md:flex flex-1 text-sm font-semibold">Tổng số đơn hàng: {totalUsers}</div>
